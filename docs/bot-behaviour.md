...
| **Guards** | Through `State::perceive_guard` (§9.2). A **seen** guard's cone is known and avoided (the danger overlay, §11.5) — *whatever shape the level deals it*, so the narrowed-cones modifier (§12.6/#495) needs no cue of its own: the bot judges the ground the overlay actually paints; a **sensed** guard is a bare position to keep away from; one that is neither is invisible. The §9.5 fading marks (`State::sense_marks`) are deliberately *not* read: a trail is the board reminding a human of what it showed a turn ago, and the bot reads the live channel directly, so using it would double-count what it already knows |
