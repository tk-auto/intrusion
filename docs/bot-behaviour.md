...
| **Contents** | *Fogged.* Intel consoles are unknown until seen and remembered after (`State::memory`). The bot cannot route to a console it has never laid eyes on — it explores to find them, exactly as a player must. The **pre-level scout** (§14 v3/#215, `--modifier scouted`) fills that memory at boot instead of by looking, so the bot plays it honestly with **no cue of its own**: what changed is the board its existing explore→take→leave loop reads. What it cannot weigh is the **price** — the sim plays single facilities and has no wallet — so a batch here measures what foreknowledge is worth, never whether it is worth three intel |
| **Guards** | Through `State::perceive_guard` (§9.2). A **seen** guard's cone is known and avoided (the danger overlay, §11.5) — *whatever shape the level deals it*, so the short-sighted-guards modifier (§12.6/#495) needs no cue of its own: the bot judges the ground the overlay actually paints; a **sensed** guard is a bare position to keep away from (`perceived_guard_cells`, which feeds the route's keep-away cost, the flee and hide-until-clear radii, the bench it takes cover behind and the `nearest_guard` every ability cue is weighed against); one that is neither is invisible. The §12.6 modifier that switches the **sense off** (§9/#493) therefore needs no cue either, and lands on the bot honestly: with it on the sensed arm returns nothing, so the bot routes past patrols it can no longer feel, exactly as the player does. The §9.5 fading marks (`State::sense_marks`) are deliberately *not* read: a trail is the board reminding a human of what it showed a turn ago, and the bot reads the live channel directly, so using it would double-count what it already knows |
