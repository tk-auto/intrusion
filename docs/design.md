...
| **Repel** | 1 turn | 8 | 40 | **[START] (#554).** A **wall you can put down in the open**. Firing stamps the `REPEL_RADIUS` (**3** **[START]**) box on **the cell you fired it from** as ground **no guard may step into**, in any state — Calm, investigating, searching or chasing alike: a route across it goes the long way round (§7.6/§10.4's *"a guard cannot get it open, so its route goes the long way round"*, read over open floor). It is **Lockdown's trade where Lockdown is inert** — a hub room, a stretch of open floor, a corridor with nothing to shut — which is why it carries Lockdown's own clocks: two abilities that buy a detour should be told apart by *where* they work, and if one is always the better press the knob to move is the radius, not the pair of clocks. A **snapshot**, not a travelling bubble: the disc stays where it fell, because a disc centred on a moving player is one no guard could ever reach him in, and §4.5's capture-is-contact is **[SETTLED]**. **You** are never refused — you walk your own field freely, in and out, which is what stops it ever walling its owner in. **It binds nobody who is already inside**: a guard standing where the disc lands is not moved, may step within it and may walk out, and only once out is it refused — the rule is a crossing of the boundary *inward* and nothing else, so *"it cannot come back"* needs nothing remembered. The consequence is the ability's main mistake: fired with a chaser at arm's length it builds a wall with the hunter inside it, spends the turn and the lockout, and you are taken. **It conceals nothing** — a guard that can see through the field sees you, steps §7.3's ladder and calls it in (§7.7) — so what gathers outside a wall nobody may cross is a **ring of guards** standing exactly where you must come out when the window drops. A guard with **no route at all holds** where it stands, exactly as one facing a sealed doorway does; the window is the only clock, so it is a wait and never a deadlock. **Every cell is released when the window ends**, expiry or early toggle-off alike (§8.2). A firing with nobody in reach is **never refused and never greyed** and costs its full turn and lockout: the only precondition worth asking would be *"is a guard near enough?"*, and answering it — every frame, for free — is a detector, not terrain (False Call's reasoning). Appendix 60. |
