...
//!
//! That is also where an alert raise's **reason** shows up when it has one (#418): the
//! raise's subordinate message follows its headline through
//! [`loudest_first`](crate::status::live_messages), so when the near line speaks
//! *"security condition 2 of 3"* the row directly beneath the band can say what did it.
//! The near line is one row and keeps the condition alone (§11.4); the *why* is what the
//! player finds on deploying. Half the ladder's triggers say nothing here, because the
//! turn's own events already report them and a reason would be that message twice over,
//! one row apart.
//!
...
mod tests {
    ...
    /// empty history behind them. The escalation it raises is a **found body**, whose
    /// own event already reports it, so no reason rides under the headline (#418).
    fn loud_step() -> State {
    ...
    fn the_near_line_counts_extra_messages_and_deploys_the_list() {
        ...
        // The find reports itself **once** (#418): the escalation it caused says no
        // reason, because the row below the band already is the reason.
        assert!(
            !row_text(&g, LOG_TOP_ROW + 1).contains("a guard found a body"),
            "no reason line restating the find"
        );
        assert_eq!(
    ...
    /// #418: an alert raise **that keeps a reason** still has something to deploy on its
    /// own. The near line speaks the condition and nothing else (§11.4, one row), so the
    /// block is exactly the one row saying what did it, plus the rule that closes it.
    ///
    /// A raise whose trigger stays silent — because the turn's own events report it —
    /// deploys nothing of its own, which is the other half of the same rule and is what
    /// the closing assertion checks.
    #[test]
    fn a_raise_on_its_own_still_deploys_its_reason() {
        ...
        // near line says the whole of it (§11.7). A body find's raise is exactly that
        // case, since `Event::BodyFound` is the reason (#418).
        assert!(assemble(live[..1].to_vec(), &MessageHistory::default()).is_empty());
        let silent = crate::status::loudest_first(&[Event::AlertRaised {
            rung: 3,
            trigger: AlertTrigger::BodyFound,
        }]);
        assert_eq!(silent.len(), 1, "the raise stands alone");
        assert!(assemble(silent, &MessageHistory::default()).is_empty());
    }
