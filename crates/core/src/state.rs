...
//!
//! This file owns the [`State`] itself, [`step`](State::step), the player phase
//! (with the §4.3 bump ladder it resolves) and the sight phase. The rest is next
//! door, each in its own module: the tuning catalogue ([`tuning`]), the public
//! [`Input`]/[`Event`]/[`Affordance`] vocabulary ([`events`]), the read surface the
//! renderer and the §13.2 bot ask ([`view`]), phase 3 with its knobs and the §7.3
//! radio net ([`guards`]), the doors' own turn ([`doors`]), the §9 sense channel
//! ([`sense`]), the ability machinery ([`abilities`], [`activation`], [`effects`],
//! [`lockdown`], [`bore`], the piloting seam [`control`]), the §7.3 reinforcements
//! ([`reinforcements`]) and the #57 auto-slide ([`traversal`]). They are all `impl State` blocks over the *same*
